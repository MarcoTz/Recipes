use super::{errors::Error, parse_steps::ParseStep, steps::parse_step, tag::parse_tags, Parse};
use recipes::{Ingredient, IngredientSection, Recipe, StepSection, TextBlock};
use std::str::Lines;

impl Parse for Recipe {
    fn parse(input: &mut String) -> Result<Recipe, Error> {
        ...
        for mut input in inputs {
            ...
            match current_step {
                ...
                ParseStep::Ingredients => {
                    let ing: Ingredient = Parse::parse(&mut input.to_owned())?;
